...
auther use the fact that $ \pi_{i+1} = \underset{\pi}{argmax}(L_\pi_i (\pi) CD_{KL}^{max}(\pi_i , \pi)) $ is one of form of lagrange mulifiplier formula, they manipulate term C to comverge more faster with take a risk of little distributional shift.

this is last formula
$$ \underset{\pi}{maximize}(\nabla_\theta L_\theta_old (\theta) |_{\theta = \theta_old} (\theta - \theta_old)) $$
subject to {1}
